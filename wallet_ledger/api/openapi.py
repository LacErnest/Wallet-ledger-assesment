...
def build_spec() -> dict:
    return {
        ...
        "servers": [{"url": "/api/v1"}],
        "tags": [
            {"name": "Accounts", "description": "Create accounts and read ledger-derived balances and history."},
            {"name": "Transfers", "description": "Move funds between accounts — atomic or two-phase (reserve/settle)."},
            {"name": "Deposits", "description": "Fund a wallet from an external provider (Stripe, PawaPay)."},
    ...
    return {
        "/accounts": {
            "post": {
                ...
                "responses": {
                    "201": _json("#/components/schemas/Account", {
                        "id": "3d1a93e3-bac5-4bc0-ba10-e0d267444e7d", "number": "2104077793",
                        "owner_id": "alice", "currency": "USD", "created_at": "2026-06-18T12:00:00+00:00"}) | {"description": "Account created"},
                    "400": {"$ref": "#/components/responses/ValidationError"},
                    "409": _error_response("An account already exists for this owner and currency", "DUPLICATE_ACCOUNT", "Un compte existe déjà pour alice en USD"),
                },
            }
        },
        "/accounts/{number}": {
            "get": {
                ...
                "responses": {
                    "200": _json("#/components/schemas/Account", {
                        "id": "3d1a93e3-bac5-4bc0-ba10-e0d267444e7d", "number": "2104077793",
                        "owner_id": "alice", "currency": "USD", "created_at": "2026-06-18T12:00:00+00:00"}) | {"description": "The account"},
        ...
        "/accounts/{number}/balance": {
            "get": {
                ...
                "parameters": [_path_param("number", "Account number")],
                "responses": {
                    "200": _json("#/components/schemas/Balance", {"account_number": "2104077793", "balance": "170.00", "currency": "USD"}) | {"description": "Current balance"},
                    "404": {"$ref": "#/components/responses/NotFound"},
                },
            }
        },
        "/accounts/{number}/transactions": {
            "get": {
                ...
                "responses": {
                    "200": _json("#/components/schemas/TransactionList", {
                        "items": [_TXN_EXAMPLE], "page": 1, "per_page": 20, "total": 1, "pages": 1}) | {"description": "Paginated transactions"},
                    "404": {"$ref": "#/components/responses/NotFound"},
                },
            }
        },
        "/transfers": {"post": _transfer_op(
        ...
        "/transfers/initiate": {"post": _transfer_op(
        ...
        "/transfers/{transaction_id}/commit": {"post": _two_phase_op(
        ...
        "/deposits": {
            "post": {
                ...
                "requestBody": {"required": True, "content": {"application/json": {
                    ...
                    "examples": {
                        "stripe": {"value": {"account_number": "2104077793", "amount": "200.00", "provider": "stripe"}},
                        "pawapay_mtn": {"value": {"account_number": "2104077793", "amount": "5000", "provider": "pawapay", "operator": "mtn", "phone_number": "237650000000"}},
                    }}}},
                "responses": {
                    "201": _json("#/components/schemas/Transaction", _TXN_EXAMPLE | {"type": "DEPOSIT", "status": "PENDING"}) | {"description": "Deposit initiated (PENDING)"},
                    "400": {"$ref": "#/components/responses/ValidationError"},
                    "404": {"$ref": "#/components/responses/NotFound"},
                },
            }
        ...
        "/payments/webhook/{provider}": {
            "post": {
                ...
                "parameters": [
                    ...
                    {"name": "Stripe-Signature", "in": "header", "required": False, "schema": {"type": "string"}, "description": "Stripe signature header (t=,v1=)."},
                    {"name": "X-Signature", "in": "header", "required": False, "schema": {"type": "string"}, "description": "HMAC-SHA256 of the raw body (PawaPay)."},
                ],
                "requestBody": {"required": True, "content": {"application/json": {
                    "example": {"depositId": "0d4f9b2e-2b8a-4f1e-9c33-2a1b6c5d4e3f", "status": "COMPLETED", "amount": "5000", "currency": "XAF"}}}},
                "responses": {
                    "200": {"description": "Deposit settled or ignored",
                            "content": {"application/json": {"example": {"transaction_id": "0d4f9b2e-...", "status": "SUCCESS"}}}},
                    "401": _error_response("Signature verification failed", "WEBHOOK_VERIFICATION_FAILED", "Signature de webhook invalide pour pawapay"),
                    "409": _error_response("Deposit already settled (replayed webhook)", "INVALID_TRANSACTION_STATE", "Transition d'état interdite"),
                    "422": _error_response("Confirmed amount/currency does not match the authorized deposit", "DEPOSIT_AMOUNT_MISMATCH", "Le montant confirmé ne correspond pas au montant autorisé"),
                },
            }
        },
        "/fx/convert": {
            "get": {
                ...
                "parameters": [
                    ...
                    {"name": "amount", "in": "query", "required": True, "schema": {"type": "string"}, "example": "108"},
                ],
                "responses": {
                    "200": {"description": "Converted amount",
                            "content": {"application/json": {"example": {"converted_amount": "100.00", "currency": "EUR"}}}},
                    "400": {"$ref": "#/components/responses/ValidationError"},
                },
            }
        },
        "/fx/transfer": {
            "post": {
                ...
                "requestBody": {"required": True, "content": {"application/json": {
                    ...
                    "example": {"sender_account_number": "2104077793", "receiver_account_number": "2511399945", "amount": "108"}}}},
                "responses": {
                    "201": _json("#/components/schemas/Transaction", _TXN_EXAMPLE | {"type": "FX_TRANSFER"}) | {"description": "FX transfer completed"},
                    "400": {"$ref": "#/components/responses/ValidationError"},
                    "404": {"$ref": "#/components/responses/NotFound"},
                    "422": _error_response("Same currency, or insufficient funds", "SAME_CURRENCY", "Transfert FX inutile : les deux comptes sont en USD"),
                },
            }
        },
    }
...
def _transfer_op(summary: str, description: str) -> dict:
    return {
        ...
        "requestBody": {"required": True, "content": {"application/json": {
            "schema": {"$ref": "#/components/schemas/TransferRequest"},
            "example": {"sender_account_number": "2104077793", "receiver_account_number": "2511399945", "amount": "30.00"}}}},
        "responses": {
            "201": _json("#/components/schemas/Transaction", _TXN_EXAMPLE) | {"description": "Transfer created"},
            "400": {"$ref": "#/components/responses/ValidationError"},
            "404": {"$ref": "#/components/responses/NotFound"},
            "422": _error_response("Insufficient funds or currency mismatch", "INSUFFICIENT_FUNDS", "Fonds insuffisants : disponible=20, demandé=50"),
        },
    }
...
def _two_phase_op(summary: str, description: str) -> dict:
    return {
        ...
        "responses": {
            "200": _json("#/components/schemas/Transaction", _TXN_EXAMPLE) | {"description": "Updated transaction"},
            "404": {"$ref": "#/components/responses/NotFound"},
            "409": _error_response("Transaction is not in a state that allows this transition", "INVALID_TRANSACTION_STATE", "Transition d'état interdite"),
            "422": _error_response("Rejected by the internal risk service", "RISK_REJECTED", "Transfert refusé par le contrôle de risque"),
        },
    }
    ...
    return {
        "schemas": {
            "CreateAccountRequest": {
                "type": "object", "required": ["owner_id", "currency"],
                "properties": {
            ...
            "TransferRequest": {
                "type": "object", "required": ["sender_account_number", "receiver_account_number", "amount"],
                "properties": {
                    "sender_account_number": {"type": "string", "example": "2104077793"},
                    "receiver_account_number": {"type": "string", "example": "2511399945"},
                    "amount": {"type": "string", "description": "Decimal string (never a float)", "example": "30.00"},
                },
            },
            "DepositRequest": {
                "type": "object", "required": ["account_number", "amount", "provider"],
                "properties": {
                    "account_number": {"type": "string", "example": "2104077793"},
                    "amount": {"type": "string", "example": "200.00"},
                    "provider": {"type": "string", "enum": ["stripe", "pawapay"]},
                    "operator": {"type": "string", "enum": ["mtn", "orange"], "description": "PawaPay only"},
                    "phone_number": {"type": "string", "description": "PawaPay only", "example": "237650000000"},
                },
            },
            "Account": {
                "type": "object",
                "properties": {
                    ...
                    "owner_id": {"type": "string"}, "currency": {"type": "string"},
            ...
            "Balance": {
                ...
                "properties": {
                    ...
                    "balance": {"type": "string", "description": "Decimal string at the currency's precision"},
            ...
            "Transaction": {
                ...
                "properties": {
                    ...
                    "type": {"type": "string", "enum": ["DEPOSIT", "TRANSFER", "FX_TRANSFER", "REVERSAL"]},
        ...
        "responses": {
            "NotFound": _error_response("Resource not found", "ACCOUNT_NOT_FOUND", "Compte introuvable : 0000000000"),
            "ValidationError": {
                "description": "Request failed validation",
                "content": {"application/json": {"schema": {"$ref": "#/components/schemas/Error"},
                            "example": {"error": "Requête invalide", "code": "VALIDATION_ERROR",
