...
class FxService:
    def __init__(self, rates: FxRateProvider | None = None, ledger: LedgerService | None = None,
    ...
    def execute_fx_transfer(self, sender_id: str, receiver_id: str, amount: Decimal,
        ...
        txn = Transaction(
            type=TransactionType.FX_TRANSFER, status=TransactionStatus.SUCCESS,
            amount=sent.amount, currency=sent.currency, correlation_id=correlation_id,
            details={
                "sender_id": sender.id, "receiver_id": receiver.id,
                "target_amount": str(received.amount), "target_currency": received.currency,
            },
        )
        db.session.add(txn)
        db.session.flush()
        ...
        self.events.publish(DomainEvent(
            TRANSFER_COMPLETED,
            {
                "transaction_id": txn.id, "sender_id": sender.id, "receiver_id": receiver.id,
                "amount": str(sent.amount), "currency": sent.currency,
                "target_amount": str(received.amount), "target_currency": received.currency,
            },
